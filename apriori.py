...
df.to_csv(r'/home/omarcc/git_environment/apriori-algrothem/upgraded_data.csv', index = False)
        ...
        for item in value:
            ...
            for element in _subsets:
                ...
                if len(rightSide) > 0:
                    ...
                    left=confidence/getSupport(rightSide,noOfReapeated,groupOfData)
                    #print(left)
                    leaverage=getSupport(item,noOfReapeated,groupOfData)-getSupport(element,noOfReapeated,groupOfData)*getSupport(rightSide,noOfReapeated,groupOfData)
                    #print(leaverage)
            ...
            # for rule, confidence in sorted(toRetRules, key=operator.itemgetter(1)):
                ...
                #     left=confidence/getSupport(post,noOfReapeated,groupOfData)
                #     leaverage=getSupport(item,noOfReapeated,groupOfData)-getSupport(post,noOfReapeated,groupOfData)*getSupport(pre,noOfReapeated,groupOfData)                                      
                    # toRetRules[i][3]=left
...
def printResults(rules):
    ...
    for rule in rules:
        ...
        # print(left)
        # print(leaverage)
        print ("Rule: %s ==> %s ,confidence= %.4f,left=  %.4f,leaverage= %.4f" % (str(f), str(sec), confidence,left,leaverage  ))
        #print ("  ,left=",rule[3])
...
if __name__ == "__main__":
    ...
    op= OptionParser()
    op.add_option('-o', '--inputFile',
                         dest='input',
                         default=None)
    op.add_option('-s', '--minSupport',
                         dest='minSupport',
                         default=0.3,
                         type='float')
    op.add_option('-c', '--minConfidence',
                         dest='minConfidence',
                         default=0.4,
                         type='float')
    ...
    rules = apriori(f, minSupport, minConfidence)
    printResults(rules)
